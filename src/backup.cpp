...
init_device(dx12device *p, HWND hwnd, UINT Width, UINT Height, UINT BufferCount)
{
	...
	for(int i = 0 ; i < BufferCount; i++) {
		...
		p->swap_chain->GetBuffer(i, IID_PPV_ARGS(&f.backbuffer));
		create_rtv(dev, f.backbuffer, get_cpu_handle(dev, f.rtv_heap, i));
		create_uav(dev, f.uavbuffer, 0, 0, get_cpu_handle(dev, f.uav_heap, i));
	...
	hr = dev->CreateRootSignature(0, signature->GetBufferPointer(), signature->GetBufferSize(), IID_PPV_ARGS(&p->rootsig));
	printf("CreateRootSignature rootsig=%p\n", p->rootsig);
	D3D12_COMPUTE_PIPELINE_STATE_DESC cpstate_desc = {};
	cpstate_desc.pRootSignature = p->rootsig;
	std::vector<uint8_t> cs;
	cpstate_desc.CS = create_shader_from_file(std::string("test.hlsl"), "CSMain", "cs_5_0", cs);
	auto status = dev->CreateComputePipelineState(&cpstate_desc, IID_PPV_ARGS(&p->pstate));
}
...
main(int argc, char *argv[])
{
	...
	for(uint64_t frame = 0 ; update_window() ; frame++) {
		...
		printf("ctx.pstate=%p\n", ctx.pstate);
		auto barrier_present_begin = get_res_barrier(f.backbuffer, D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_RENDER_TARGET);
		auto barrier_present_end = get_res_barrier(f.backbuffer, D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_COMMON);
		std::vector<D3D12_CPU_DESCRIPTOR_HANDLE> rtv_handles = { get_cpu_handle(ctx.device, f.rtv_heap, frame % BufferCount) };
		...
		cmdlist->SetPipelineState(ctx.pstate);

		cmdlist->ResourceBarrier(1, &barrier_present_begin);
		...
		for(auto hcpu : rtv_handles) {
			...
			cmdlist->ClearRenderTargetView(hcpu, clear_color[frame % 2], 0, NULL);
		}

		cmdlist->Dispatch(8, 8, 0);
		//copy_res_data(dev, cmdlist, f.backbuffer, f.uavbuffer);
		cmdlist->ResourceBarrier(1, &barrier_present_end);
		cmdlist->Close();
